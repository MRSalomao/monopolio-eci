...
public class Defender extends Creature
{

	public Defender(AnimatedObject srcObject)
	{
		super(srcObject);
	...
	{
		for (Creature creature2 : Level.getSharedInstance().enemyDic.values())
		{
			Gdx.app.log("defender - "+creatureID, "my target's pos "+target.position2D);
			Gdx.app.log("defender - "+creatureID, "my pos "+position2D);
	...
	{
		for (Creature enemy : Level.getSharedInstance().enemyDic.values())
		{
			if (enemy.position2D.dst(this.position2D) < lineOfSight)
			{
				return enemy;
			}
		}
